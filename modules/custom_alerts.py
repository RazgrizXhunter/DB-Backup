...
class Custom_alerts:
	...
	def space_alert(self):
		...
		free = File_manager.get_free_disk_space()
		total = File_manager.get_total_disk_space()

		logger.info(f"Executing storage space check.\n\tFree space: {File_manager.to_human_readable_size(free)}\n\tTotal space: {File_manager.to_human_readable_size(total)}")

		critical_percentage = self.alerts["space_critical"]
		critical_level = round(total * (critical_percentage / 100))

		logger.debug(f"Critical storage level set on: {File_manager.to_human_readable_size(critical_level)}")
